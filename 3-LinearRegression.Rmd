...
```{r}
ggplot(lm_fit, aes(x = lstat, y = medv))  +
...
```
```{r}
ggplot(lm_fit, aes(x = lm_fit$fitted.values,
                 y = lm_fit$residuals)) +
...
```{r}
ggplot(lm_fit, aes(x = predict(lm_fit), y = residuals(lm_fit))) +
  geom_point(shape = 21, fill = NA, size = 2)
ggplot(lm_fit, aes(x = predict(lm_fit), y = rstudent(lm_fit))) +
  geom_point(shape = 21, fill = NA, size = 2)
```
...
```{r}
ggplot(lm_fit) +
  geom_point(aes(x = 1:length(hatvalues(lm_fit)),
                 y = hatvalues(lm_fit))) +
  labs(x = "Index")
```
...
```{r}
ggplot(lm_fit) +
  geom_point(aes(x = horsepower, y = mpg), color = "green") +
...
```{r}
ggplot(lm_fit) +
  geom_point(aes(x = predict(lm_fit),
                 y = residuals(lm_fit)),
             shape = 21,
             size = 2, fill = NA)
```
